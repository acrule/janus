...
def check_for_nb_diff(t, hashed_full_path, cells, db):
    ...
    if( not last_nb_config ):
        for c in cells:
            ...
            cell_id = c.metadata.janus.cell_id
            version_id = uuid.uuid4().hex[0:7]
            cell_data = c
            db.record_cell(cell_id, version_id, cell_data)
    ...
    last_cell_order = last_config[2]
    last_version_order = last_config[3]
    last_cells = db.get_cell_versions(last_version_order)
    # for each cell in the current notebook
    for c in cells:
        # check if cell with same cell_id was in last nb
        cell_id = c.metadata.janus.cell_id
        version_id = uuid.uuid4().hex[0:7]
        cell_data = c

        previous_versions = [pc[2] for pc in last_cells where pc[1] == cell_id]
        if len(previous_versions) > 0:
            previous_version = previous_versions[0]
            if not cells_different(previous_version[3], c):
                version_id = previous_version[2]
                new_cell_order.append(cell_id)
                new_version_order.append(version_id)
                continue

        older_versions = db.get_previous_cell_versions(cell_id)
        for older_version in older_versions:
            if not cells_different(older_version[3], c):
